...
int main()
{
    ...
    FILE* out = fopen("data.1.c012", "wb");
    int seek_ret=BIO_seek(cipherBio,371-3-16);
    if(seek_ret!=0){
    	printf("GmCAJDCY seek_ret %d\n",seek_ret);
    	return 1;
    }
    BIO_read(cipherBio,buffer,3);
    ...
    seek_ret=fseek(in0,371,SEEK_SET);
    if(seek_ret!=0){
	...
	FILE* out0=fopen("data.0.c012", "wb");
